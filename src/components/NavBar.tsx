        ...
        <div className="fixed top-0 left-0 w-full z-50 bg-transparent">
            <div className="flex items-center justify-between px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
                ...
                <nav className="hidden md:flex items-center gap-8 text-sm">
                    {navLinks.map((link) => (
                        <a key={link.label} href={link.href} className="text-white/70 hover:text-white/80">
                            {link.label}
                        </a>
                    ))}
                </nav>
            ...
            {/* Mobile Menu */}
            <div className={`${mobileOpen ? 'w-full' : 'w-0'} md:hidden fixed top-0 left-0 h-screen bg-black/50 backdrop-blur transition-all duration-300 overflow-hidden`}>
                <div className="flex flex-col items-center justify-center h-full gap-8 text-lg">
                    {navLinks.map((link) => (
                        <a key={link.label} href={link.href} onClick={() => setMobileOpen(false)} className="text-white/70 hover:text-white/80">
                            {link.label}
                        </a>
                    ))}
                    <button id="close-menu" onClick={() => setMobileOpen(false)} className="bg-gray-900 hover:bg-gray-800 text-white p-2 rounded-md aspect-square font-medium transition">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
